...
export class ULabel {
  ...
  static create_listeners(ul) {
      ...
      switch (e.key) {
        ...
        case ul.config.create_bbox_on_initial_crop:
          if (current_subtask.state.annotation_mode === "bbox") {
            ...
            ul.create_annotation(current_subtask.state.annotation_mode, [
              bbox_top_left,
              bbox_bottom_right,
            ]);
          }
    ...
    $(document).on(
      ...
      () => {
        // Show thumbnail for idd
        if (
          ul.subtasks[ul.state["current_subtask"]]["state"]["idd_visible"] &&
          !ul.subtasks[ul.state["current_subtask"]]["state"]["idd_thumbnail"]
        ) {
          return;
        }
    ...
    $(document).on(
      "click.ulabel",
      "#" + ul.config["container_id"] + " a.id-dialog-clickable-indicator",
      (e) => {
        let crst = ul.state["current_subtask"];
        if (!ul.subtasks[crst]["state"]["idd_thumbnail"]) {
    ...
    for (const class_definition of raw_subtask_json.classes) {
      ...
      switch (typeof class_definition) {
        ...
        case "object":
          ...
          name =
            class_definition.name ?? `Class ${ulabel.valid_class_ids.length}`;
          ...
          color =
            class_definition.color ?? COLORS[ulabel.valid_class_ids.length];
  ...
  constructor(kwargs) {
    ...
    for (const st in this.subtasks) {
      for (
        ...
        let amd =
          this.subtasks[st]["annotations"]["access"][aid]["spatial_type"];
      ...
      switch (toolbox_name) {
        case "FilterDistance":
          ...
          filter_points_distance_from_line(this, true, null, {
            should_redraw:
              this.config.distance_filter_toolbox_item.filter_on_load,
            multi_class_mode:
              this.config.distance_filter_toolbox_item.multi_class_mode,
  ...
  set_annotation_mode(annotation_mode) {
    this.subtasks[this.state["current_subtask"]]["state"]["annotation_mode"] =
      annotation_mode;
    this.update_annotation_mode();
  }
  ...
  set_and_update_annotation_mode(annotation_mode) {
    ...
    if (
      !this.subtasks[this.state["current_subtask"]]["allowed_modes"].includes(
        annotation_mode,
      )
    ) {
      console.warn(
  ...
  get_empirical_scale() {
    ...
    return (
      $("#" + this.config["imwrap_id"]).width() / this.config["image_width"]
    );
  }
  ...
  get_next_available_canvas_id(subtask = null) {
    ...
    for (let i = 0; i < canvas_ids.length; i++) {
      ...
      if (
        this.subtasks[subtask]["state"]["annotation_contexts"][canvas_ids[i]][
          "annotation_ids"
        ].length < this.config.n_annos_per_canvas
      ) {
        return canvas_ids[i];
      }
    }
  ...
  register_nonspatial_redraw_start(subtask) {
    ...
    if (nonsp_window.length) {
      $(`div#fad_st__${subtask} div.fad_annotation_rows div.fad_row`).each(
        (idx, val) => {
  ...
  redraw_all_annotations_in_annotation_context(
    ...
    for (const annid of this.subtasks[subtask]["state"]["annotation_contexts"][
      canvas_id
    ]["annotation_ids"]) {
  ...
  redraw_multiple_spatial_annotations(
    ...
    for (const annid of annotation_ids) {
      unique_contexts.add(
        this.subtasks[subtask]["annotations"]["access"][annid]["canvas_id"],
      );
    }
  ...
  create_polygon_ender(gmx, gmy, polygon_id) {
    ...
    this.subtasks[this.state["current_subtask"]]["state"]["visible_dialogs"][
      ender_id
    ] = {
  ...
  toggle_brush_mode(mouse_event) {
    ...
    let is_in_polygon_mode =
      this.subtasks[current_subtask]["state"]["annotation_mode"] === "polygon";
    ...
    if (is_in_polygon_mode) {
      ...
      if (this.subtasks[current_subtask]["state"]["is_in_brush_mode"]) {
        ...
        if (
          this.subtasks[current_subtask]["state"]["starting_complex_polygon"]
        ) {
          this.undo(true);
        }
        // Show brush circle
  ...
  create_brush_circle(gmx, gmy) {
    ...
    $("#" + brush_circle_id).css({
      ...
      "background-color": this.subtasks[this.state["current_subtask"]]["state"][
        "is_in_erase_mode"
      ]
        ? "red"
  ...
  merge_polygon_complex_layer(
    ...
    if (
      ...
      const og_polygon_spatial_data = ULabelAnnotation.get_polygon_spatial_data(
        annotation,
        true,
      );
      if (layer_idx === null) {
  ...
  delete_annotations_in_polygon(delete_annid, redo_payload = null) {
    ...
    for (let [annid, annotation] of Object.entries(annotations)) {
      ...
      switch (spatial_type) {
        ...
        case "tbar":
          ...
          switch (spatial_type) {
            case "bbox":
              simple_polygon = GeometricUtils.bbox_to_simple_polygon(
                annotation["spatial_payload"],
              );
              break;
            case "tbar":
              simple_polygon = GeometricUtils.tbar_to_simple_polygon(
                annotation["spatial_payload"],
              );
              break;
          }
    ...
    if (!redoing) {
      ...
      this.subtasks[this.state["current_subtask"]]["annotations"]["ordering"] =
        this.subtasks[this.state["current_subtask"]]["annotations"][
          "ordering"
  ...
  remove_recorded_events_for_annotation(annotation_id) {
    ...
    for (let action of this.subtasks[this.state["current_subtask"]]["actions"][
      "stream"
    ]) {
  ...
  show_edit_suggestion(nearest_point, currently_exists) {
    ...
    this.subtasks[this.state["current_subtask"]]["state"]["visible_dialogs"][
      esid
    ]["left"] = nearest_point["point"][0] / this.config["image_width"];
    this.subtasks[this.state["current_subtask"]]["state"]["visible_dialogs"][
      esid
    ]["top"] = nearest_point["point"][1] / this.config["image_height"];
    this.reposition_dialogs();
  }
  ...
  show_id_dialog(gbx, gby, active_ann, thumbnail = false, nonspatial = false) {
    ...
    let idd_niu_id =
      this.subtasks[this.state["current_subtask"]]["state"]["idd_id_front"];
    let new_height = $(
    ...
    if (nonspatial) {
      ...
      let top_c =
        new_home.offset().top - fad_st.offset().top + ofst + new_height / 2;
      let left_c =
      ...
      switch (spatial_type) {
        ...
        case "polyline":
          ...
          for (let i = 0; i < n_iters; i++) {
            ...
            access_idx =
              spatial_type === "polygon" ? [i, npi["access"]] : npi["access"];
            if (npi["distance"] < ret["distance"]) {
              ret["annid"] = edid;
  ...
  record_finish(actid) {
    // TODO(3d)
    let i =
      this.subtasks[this.state["current_subtask"]]["actions"]["stream"].length -
      1;
  ...
  begin_annotation(mouse_event, redo_payload = null) {
    ...
    this.subtasks[subtask]["annotations"]["access"][unq_id]["annotation_meta"] =
    ...
    if (
      (annotation_mode === "polygon" || annotation_mode === "delete_polygon") &&
  ...
  begin_annotation__undo(undo_payload) {
    ...
    const unq_id = undo_payload.actid;
    ...
    current_subtask["annotations"]["ordering"] = current_subtask["annotations"][
      "ordering"
    ].filter((id) => id !== unq_id);
  ...
  rebuild_containing_box(actid, ignore_final = false, subtask = null) {
    ...
    const spatial_type =
      this.subtasks[subtask]["annotations"]["access"][actid]["spatial_type"];
    ...
    if (spatial_type === "polygon") {
      ...
      for (let active_spatial_payload of this.subtasks[subtask]["annotations"][
        "access"
    ...
    if (spatial_type) {
      let line_size =
        this.subtasks[subtask]["annotations"]["access"][actid]["line_size"];
      this.subtasks[subtask]["annotations"]["access"][actid]["containing_box"][
        "tlx"
      ] -= 3 * line_size;
      this.subtasks[subtask]["annotations"]["access"][actid]["containing_box"][
      ...
      switch (spatial_type) {
        ...
        case "delete_polygon":
          ...
          if (n_kpts > 0) {
            ...
            ender_thresh =
              $("#ender_" + actid).width() / (2 * this.get_empirical_scale());
  ...
  begin_brush(mouse_event) {
    ...
    if (brush_cand_active_id !== null) {
      ...
      current_subtask["state"]["id_payload"] = JSON.parse(
        JSON.stringify(
          current_subtask["annotations"]["access"][brush_cand_active_id][
            "classification_payloads"
          ],
        ),
      );
      ...
      this.record_action({
        ...
        undo_payload: {
          ...
          polygon_spatial_data: ULabelAnnotation.get_polygon_spatial_data(
            current_subtask["annotations"]["access"][brush_cand_active_id],
          ),
        },
      });
  ...
  edit_annotation(mouse_event) {
    ...
    if (active_id && active_id !== null) {
      ...
      // TODO(3d)
      switch (
        current_subtask["annotations"]["access"][active_id]["spatial_type"]
      ) {
        case "bbox":
        case "tbar":
        case "polygon":
  ...
  edit_annotation__undo(undo_payload) {
    ...
    const undo_location = [undo_payload.starting_x, undo_payload.starting_y];
  ...
  finish_annotation(mouse_event) {
    ...
    switch (annotation["spatial_type"]) {
      case "polygon":
        ...
        start_pt = [active_spatial_payload[0][0], active_spatial_payload[0][1]];
    ...
    if (record_action) {
      ...
      while (current_subtask["actions"]["stream"].length > 0) {
        ...
        if (action.act_type === "begin_annotation") {
          ...
          redo_payload.polygon_spatial_data =
            ULabelAnnotation.get_polygon_spatial_data(annotations[active_id]);
          break;
        }
      }
  ...
  finish_annotation__undo(undo_payload) {
    ...
    this.suggest_edits(this.state["last_move"]);
    // Filter points if necessary
    this.update_filter_distance(undo_payload.actid, false);
  ...
  finish_edit() {
    ...
    let actid =
      this.subtasks[this.state["current_subtask"]]["state"]["active_id"];
    const access_str =
      this.subtasks[this.state["current_subtask"]]["state"]["edit_candidate"][
        "access"
      ];
    let layer_idx;
    switch (
  ...
  finish_move(mouse_event) {
    ...
    const diffY =
      ...
      this.state["zoom_val"];
    const diffZ =
      this.state["current_frame"] - this.drag_state["move"]["mouse_start"][2];
    ...
    for (let i = 0; i < n_iters; i++) {
      ...
      if (
        spatial_type === "polygon" &&
        active_spatial_payload[0] === active_spatial_payload[n_points - 1]
      ) {
        n_points -= 1;
      }
  ...
  move_annotation__undo(undo_payload) {
    ...
    const active_id = undo_payload.move_candidate["annid"];
    const spatial_type = annotations[active_id]["spatial_type"];
    const spatial_payload = annotations[active_id]["spatial_payload"];
    let active_spatial_payload = spatial_payload;
  ...
  move_annotation__redo(redo_payload) {
    ...
    const active_id = redo_payload.move_candidate["annid"];
    const spatial_type = annotations[active_id]["spatial_type"];
    const spatial_payload = annotations[active_id]["spatial_payload"];
    let active_spatial_payload = spatial_payload;
  ...
  suggest_edits(mouse_event = null, nonspatial_id = null) {
    ...
    if (
      ...
      if (nonspatial_id === null) {
        ...
        const nearest_active_keypoint = this.get_nearest_active_keypoint(
          global_x,
          global_y,
          dst_thresh,
        ...
        if (
          ...
          const nearest_segment_point = this.get_nearest_segment_point(
            global_x,
            global_y,
            Infinity,
            edit_candidates["candidate_ids"],
  ...
  get_global_mouse_x(mouse_event) {
    ...
    const raw =
      (mouse_event.pageX - annbox.offset().left + annbox.scrollLeft()) / scale;
  ...
  get_global_mouse_y(mouse_event) {
    ...
    const annbox = $("#" + this.config["annbox_id"]);
    const raw =
      (mouse_event.pageY - annbox.offset().top + annbox.scrollTop()) / scale;
    // return Math.round(raw);
    return raw;
  }
  ...
  drag_rezoom(mouse_event) {
    ...
    this.state["zoom_val"] =
      ...
      Math.pow(1.1, -(aY - this.drag_state["zoom"]["mouse_start"][1]) / 10);
    this.rezoom(
  ...
  get_annotations(subtask) {
    ...
    for (
      ...
      if (
        id != this.subtasks[this.state["current_subtask"]]["state"]["active_id"]
      ) {
  ...
  set_annotations(new_annotations, subtask) {
    ...
    for (
      ...
      if (
        !NONSPATIAL_MODES.includes(
          this.subtasks[subtask]["annotations"]["access"][id]["spatial_type"],
        )
      ) {
