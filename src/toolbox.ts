 ...
 */
String.prototype.replaceLowerConcat = function (
    before: string,
    after: string,
    concat_string: string = null,
) {
    if (typeof concat_string === "string") {
        return this.replaceAll(before, after)
            .toLowerCase()
...
export class Toolbox {
    ...
    public static create_toolbox(
        ulabel: ULabel,
        toolbox_item_order: unknown[],
    ) {
        ...
        for (let i = 0; i < toolbox_item_order.length; i++) {
            ...
            const toolbox_item_class =
                ulabel.config.toolbox_map.get(toolbox_key);

            if (args == null) {
                toolbox_instance_list.push(new toolbox_item_class(ulabel));
            } else {
                toolbox_instance_list.push(
                    new toolbox_item_class(ulabel, args),
                );
            }
        }
    ...
    public get_toolbox_tabs(ulabel: ULabel): string {
        ...
        for (const st_key in ulabel.subtasks) {
            ...
            const current_tab = new ToolboxTab([], subtask, st_key, selected);
...
export class ModeSelectionToolboxItem extends ToolboxItem {
    constructor(public ulabel: ULabel) {
        ...
        $(document).on("click.ulabel", "a.md-btn", (e) => {
            ...
            if (
                target_jq.hasClass("sel") ||
                ulabel.subtasks[current_subtask]["state"]["is_in_progress"]
            )
                return;

            // Get the new mode and set it to ulabel's current mode
            const new_mode = target_jq.attr("id").split("--")[1];
            ulabel.subtasks[current_subtask]["state"]["annotation_mode"] =
                new_mode;
            ...
            if (new_mode === "polygon") {
                ...
                if (
                    ulabel.subtasks[current_subtask]["state"][
                        "is_in_erase_mode"
                    ]
                ) {
        ...
        $(document).on("keypress.ulabel", (e) => {
            ...
            if (e.key == ulabel.config.toggle_annotation_mode_keybind) {
                ...
                for (const idx in Array.from(
                    document.getElementsByClassName("md-btn"),
                )) {
                    // Grab mode button
                    const mode_button = <HTMLElement>(
                        document.getElementsByClassName("md-btn")[idx]
                    );
                ...
                const selected_mode_button = <HTMLAnchorElement>(
                    Array.from(document.getElementsByClassName("md-btn sel"))[0]
                ); // There's only ever going to be one element in this array, so grab the first one
...
export class BrushToolboxItem extends ToolboxItem {
    ...
    public after_init() {
        ...
        if (
            this.ulabel.subtasks[this.ulabel.state["current_subtask"]].state[
                "annotation_mode"
            ] !== "polygon"
        ) {
            BrushToolboxItem.hide_brush_toolbox_item();
...
export class ClassCounterToolboxItem extends ToolboxItem {
    ...
    update_toolbox_counter(subtask: ULabelSubtask): void {
        ...
        for (i = 0; i < annotation_ids.length; i++) {
            ...
            if (current_annotation.deprecated === false) {
                for (
                    j = 0;
                    j < current_annotation.classification_payloads.length;
                    j++
                ) {
                    current_payload =
                        current_annotation.classification_payloads[j];
        ...
        this.inner_HTML =
            `<p class="tb-header">Annotation Count</p>` + `<p>${f_string}</p>`;
    }
    ...
    public redraw_update(ulabel: ULabel) {
        ...
        $(
            "#" + ulabel.config["toolbox_id"] + " div.toolbox-class-counter",
...
export class AnnotationResizeItem extends ToolboxItem {
    ...
    constructor(ulabel: ULabel) {
        ...
        for (const subtask in ulabel.subtasks) {
            const cached_size_property = ulabel.subtasks[
                subtask
            ].display_name.replaceLowerConcat(" ", "-", "-cached-size");
            const size_cookie = this.read_size_cookie(ulabel.subtasks[subtask]);
            if (size_cookie != null && size_cookie != "NaN") {
                this.update_annotation_size(
                    ulabel,
                    ulabel.subtasks[subtask],
                    Number(size_cookie),
                );
                ...
                this.update_annotation_size(
                    ulabel,
                    ulabel.subtasks[subtask],
                    ulabel.config.default_annotation_size,
                );
                this[cached_size_property] =
                    ulabel.config.default_annotation_size;
            } else {
    ...
    private add_event_listeners() {
        $(document).on("click.ulabel", ".annotation-resize-button", (event) => {
            ...
            // Use the button id to get what size to resize the annotations to
            const annotation_size = <ValidResizeValues>(
                button.attr("id").slice(18)
            );
        ...
        $(document).on("keydown.ulabel", (event) => {
            ...
            switch (event.key) {
                case this.keybind_configuration.annotation_vanish.toUpperCase():
                    this.update_all_subtask_annotation_size(
                        this.ulabel,
                        ValidResizeValues.VANISH,
                    );
                    break;
     ...
     */
    public update_annotation_size(
        ulabel: ULabel,
        subtask: ULabelSubtask,
        size: number | ValidResizeValues,
    ): void {
        ...
        const subtask_cached_size = subtask.display_name.replaceLowerConcat(
            " ",
            "-",
            "-cached-size",
        );
        const subtask_vanished_flag = subtask.display_name.replaceLowerConcat(
            " ",
            "-",
            "-vanished",
        );
        ...
        switch (size) {
            ...
            case ValidResizeValues.VANISH:
                if (this[subtask_vanished_flag]) {
                    ...
                    this.loop_through_annotations(
                        subtask,
                        this[subtask_cached_size],
                        "=",
                    );
            ...
            default:
                console.error(
                    "update_annotation_size called with unknown size",
                );
        }
    ...
    public loop_through_annotations(
        subtask: ULabelSubtask,
        size: number,
        operation: "=" | "+" | "-",
    ) {
        for (const annotation_id in subtask.annotations.access) {
            switch (operation) {
                ...
                case "-":
                    ...
                    if (
                        subtask.annotations.access[annotation_id].line_size -
                            size <=
                        0.01
                    ) {
                        subtask.annotations.access[annotation_id].line_size =
                ...
                default:
                    throw Error(
                        "Invalid Operation given to loop_through_annotations",
                    );
            }
        }

        if (subtask.annotations.ordering.length > 0) {
            this.set_size_cookie(
                subtask.annotations.access[subtask.annotations.ordering[0]]
                    .line_size,
                subtask,
            );
        }
...
export class RecolorActiveItem extends ToolboxItem {
    ...
    private save_local_storage_color(
    ...
    private read_local_storage(): void {
        ...
        for (const class_id of this.ulabel.valid_class_ids) {
            ...
            const color = get_local_storage_item(
                `RecolorActiveItem-${class_id}`,
            );
        ...
        this.gradient_turned_on = get_local_storage_item(
            "RecolorActiveItem-Gradient",
        );
    }
    ...
    private replace_color_pie(): void {
        ...
        const current_subtask: ULabelSubtask =
            this.ulabel.subtasks[current_subtask_key];

        // Get the back and front id dialog's ids
        const id_dialog_id: string = current_subtask.state.idd_id;
        const front_id_dialog_id =
            this.ulabel.subtasks[current_subtask_key].state.idd_id_front;
        ...
        const subtask_dialog_container_jq = $(
            "#dialogs__" + current_subtask_key,
        );
        const id_dialog_container = $(`#id_dialog__${current_subtask_key}`);
        const front_subtask_dialog_container_jq = $(
            "#front_dialogs__" + current_subtask_key,
        );
        const front_id_dialog_container = $(
            `#id_front_dialog__${current_subtask_key}`,
        );
        ...
        const dialog_html_v2 = get_idd_string(
            id_dialog_id,
            width,
            this.ulabel.subtasks[current_subtask_key].class_ids,
            inner_radius,
            color_info,
        );
        ...
        $(".id_dialog").on("mousemove.ulabel", function (mouse_event) {
            if (
                !that.ulabel.subtasks[current_subtask_key].state.idd_thumbnail
            ) {
    ...
    private update_color(
        ...
        const button_color_square = <HTMLDivElement>(
            document.querySelector(`#toolbox_sel_${class_id} > div`)
        );
    ...
    private add_event_listeners(): void {
        ...
        $(document).on("input.ulabel", "input.color-change-picker", (event) => {
            ...
            const color_picker_container = <HTMLDivElement>(
        ...
        $(document).on("input.ulabel", "#gradient-slider", (event) => {
            ...
            $("div.gradient-slider-value-display").text(
     ...
     */
    private redraw(
        wait_time: number = 100,
        redraw_all_annotations: boolean = false,
    ): void {
        ...
        if (redraw_all_annotations) {
            ...
            const current_subtask_key: string =
                this.ulabel.state.current_subtask;
...
export class KeypointSliderItem extends ToolboxItem {
    ...
    private filter_annotations(
        ...
        const annotations_ids_to_redraw_by_subtask: {
            [key: string]: string[];
        } = {};
        ...
        const point_and_line_annotations =
            get_point_and_line_annotations(ulabel);
        for (const annotation of point_and_line_annotations[0]) {
            ...
            const should_deprecate: boolean = this.filter_function(
                confidence,
                filter_value,
            );
            ...
            if (
                ...
                annotations_ids_to_redraw_by_subtask[
                    annotation.subtask_key
                ].push(annotation.id);
            }
        }
        ...
        if (redraw) {
            ...
            for (const subtask_key in annotations_ids_to_redraw_by_subtask) {
                ulabel.redraw_multiple_spatial_annotations(
                    annotations_ids_to_redraw_by_subtask[subtask_key],
                    subtask_key,
                );
            }
...
export class FilterPointDistanceFromRow extends ToolboxItem {
    ...
    constructor(ulabel: ULabel, kwargs: { [name: string]: any } = null) {
        ...
        this.collapse_options = get_local_storage_item(
            "filterDistanceCollapseOptions",
        );
        ...
        const show_overlay = get_local_storage_item(
            "filterDistanceShowOverlay",
        );
        // Guard against null values
        this.show_overlay =
            show_overlay !== null ? show_overlay : this.show_overlay;
        ...
        const filter_during_polyline_move = get_local_storage_item(
            "filterDistanceFilterDuringPolylineMove",
        );
        // Guard against null values
        this.filter_during_polyline_move =
            filter_during_polyline_move !== null
    ...
    private add_event_listeners() {
        ...
        $(document).on(
            "click.ulabel",
            "fieldset.filter-row-distance-options > legend",
            () => this.toggleCollapsedOptions(),
        );

        // Whenever the multi-class filtering checkbox is clicked, switch the displayed filter mode
        $(document).on(
            "click.ulabel",
            "#filter-slider-distance-multi-checkbox",
            (event) => {
        ...
        $(document).on(
            "change.ulabel",
            "#filter-slider-distance-toggle-overlay-checkbox",
            (event) => {
        ...
        $(document).on(
            "change.ulabel",
            "#filter-slider-distance-filter-during-polyline-move-checkbox",
            (event) => {
        ...
        $(document).on("keypress.ulabel", (event) => {
            ...
            const show_overlay_checkbox: HTMLInputElement =
    ...
    private toggleCollapsedOptions() {
        ...
        $("fieldset.filter-row-distance-options").toggleClass(
            "ulabel-collapsed",
        );
    ...
    private create_overlay() {
        ...
        const line_annotations: ULabelAnnotation[] =
            get_point_and_line_annotations(this.ulabel)[1];
        ...
        this.overlay = new FilterDistanceOverlay(
            ...
            this.ulabel.config["image_height"] *
    ...
    private createMultiFilterHTML(): string {
        ...
        for (let idx = 0; idx < class_defs.length; idx++) {
            ...
            if (this.default_values[current_id] !== undefined) {
                default_value =
                    this.default_values[current_id].distance.toString();
            } else {
                default_value =
                    this.default_values.closest_row.distance.toString();
            }
            ...
            const multi_class_slider_instance = new SliderHandler({
                ...
                slider_event: () =>
                    filter_points_distance_from_line(this.ulabel, false),
            });
...
export class SubmitButtons extends ToolboxItem {
    ...
    constructor(ulabel: ULabel) {
        ...
        for (const idx in this.submit_buttons) {
            ...
            $(document).on(
                "click.ulabel",
                "#" +
                    this.submit_buttons[idx].name.replaceLowerConcat(" ", "-"),
                async () => {
                    // Grab the button
                    ...
                    const submit_button_elements = <HTMLButtonElement[]>(
                        Array.from(
                    ...
                    for (const stkey in ulabel.subtasks) {
                        ...
                        for (
                            let i = 0;
                            i <
                            ulabel.subtasks[stkey]["annotations"]["ordering"]
                                .length;
                            i++
                        ) {
                            const annotation =
                                ulabel.subtasks[stkey]["annotations"]["access"][
                                    ulabel.subtasks[stkey]["annotations"][
                                        "ordering"
                                    ][i]
                                ];
    ...
    private sort_buttons_by_row_number() {
        ...
        const row_numbers: Set<number> = new Set(
            this.submit_buttons.map((button) =>
                button.row_number ? button.row_number : 0,
            ),
        );
        // Sort the row numbers
        const sorted_row_numbers: number[] = Array.from(row_numbers).sort(
            (a, b) => a - b,
        );
