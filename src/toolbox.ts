...
import { DEFAULT_FILTER_DISTANCE_CONFIG, DEFAULT_CONFIDENCE_FILTER_CONFIG } from "./configuration";
...
export class Toolbox {
    ...
    public get_toolbox_tabs(ulabel: ULabel): string {
        ...
        for (const st_key in ulabel.subtasks) {
            const selected = st_key == ulabel.get_current_subtask_key();
 ...
 */
export class ConfidenceFilterItem extends ToolboxItem {
    public ulabel: ULabel;
    public config!: ConfidenceFilterConfig;
    public name!: string;
    public filter_min!: number;
    public filter_max!: number;
    public step_value!: number;
    public default_values!: ConfidenceFilterClasses;
    public multi_class_mode!: boolean;
    public disable_multi_class_mode!: boolean;
    ...
    protected deprecated_by_key: ValidDeprecatedBy = "spatial_confidence_filter";

    // DOM naming used to build and read this item's sliders
    protected component_prefix: string = "confidence-filter";
    protected slider_class: string = "confidence-filter-slider";
    ...
    constructor(ulabel: ULabel, kwargs: { [name: string]: any } | null = null) {
        ...
        if (this.get_toolbox_item_type() !== "ConfidenceFilter") return;

        // Get this component's config from ulabel's config, defaulting any missing keys
        this.config = this.ulabel.config.confidence_filter_toolbox_item ?? {};
        for (const key in DEFAULT_CONFIDENCE_FILTER_CONFIG) {
            if (!Object.prototype.hasOwnProperty.call(this.config, key)) {
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                (this.config as any)[key] = (DEFAULT_CONFIDENCE_FILTER_CONFIG as any)[key];
            }
        }
        ...
        this.multi_class_mode = this.config.multi_class_mode!;
        this.disable_multi_class_mode = this.config.disable_multi_class_mode!;
    ...
    protected add_styles() {
        const css = `
            #toolbox div.confidence-filter {
                text-align: left;
            }

            #toolbox div.confidence-filter fieldset.confidence-filter-options {
                display: inline-block;
                position: relative;
                left: 1rem;
            ...
            #toolbox div.confidence-filter fieldset.confidence-filter-options legend {
                border-radius: 0.1rem;
                padding: 0.1rem 0.3rem;
                cursor: pointer;
            }

            #toolbox div.confidence-filter fieldset.confidence-filter-options legend:hover {
                background-color: rgba(128, 128, 128, 0.3);
            }

            #toolbox div.confidence-filter fieldset.confidence-filter-options.ulabel-collapsed {
                border: none;
                margin-bottom: 0;
                padding: 0;
            ...
            #toolbox div.confidence-filter fieldset.confidence-filter-options.ulabel-collapsed :not(legend) {
                display: none;
            }

            #toolbox div.confidence-filter fieldset.confidence-filter-options input[type="checkbox"] {
                margin: 0;
            }

            #toolbox div.confidence-filter fieldset.confidence-filter-options label {
                position: relative;
                top: -0.2rem;
                font-size: smaller;
            }`;

        const style_id = "confidence-filter-toolbox-item-styles";
    ...
    private add_event_listeners(): void {
        ...
        $(document).on("click.ulabel", "fieldset.confidence-filter-options > legend", () => this.toggleCollapsedOptions());
    ...
    private toggleCollapsedOptions(): void {
        $("fieldset.confidence-filter-options").toggleClass("ulabel-collapsed");
     ...
     */
    public get_filter_values(): ConfidenceFilterClasses {
        // Seed with defaults so filtering works before the sliders are rendered (e.g. on load)
        const values: ConfidenceFilterClasses = { all: { confidence: this.default_values.all.confidence } };
    ...
    public get_html(): string {
        ...
        <div class="confidence-filter">
            ...
            ${options_html}
            <div id="${this.component_prefix}-single-class-mode" class="${!this.multi_class_mode ? "" : "ulabel-hidden"}">
                ${single_class_slider.getSliderHTML()}
            </div>
            <div id="${this.component_prefix}-multi-class-mode" class="${this.multi_class_mode ? "" : "ulabel-hidden"}">
                ${multi_class_html}
            </div>
        </div>
        `;
    ...
    /**
     * Get the current confidence filter slider values by reading the DOM slider elements.
     *
     * @returns A map of class identifiers to their threshold values, or null if no sliders are found
     */
    public get_current_values(): ConfidenceFilterClasses | null {
        const all_slider = document.querySelector<HTMLInputElement>(`#${this.component_prefix}-all`);
        if (all_slider === null) return null;

        const values: ConfidenceFilterClasses = { all: { confidence: all_slider.valueAsNumber } };
 ...
 *
 * @deprecated Use {@link ConfidenceFilterItem} (`AllowedToolboxItem.ConfidenceFilter`) instead,
 ...
 */
export class KeypointSliderItem extends ConfidenceFilterItem {
    public html!: string;
    public inner_HTML: string;
